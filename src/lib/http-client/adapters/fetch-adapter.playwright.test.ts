/**
 * Playwright browser integration tests for FetchAdapter and HTTPClient + FetchAdapter.
 *
 ...
 *      fetch-adapter.browser-script.ts with `bun build --target browser`,
 *      and launches headless Chromium.
 *   2. The bundled script is inlined into a page. The real server URL is
 *      injected as window.adapterBrowserTestBaseURL so the browser hits the
 *      server directly — no Playwright route interception, no fake domains.
 *   3. The script runs all test scenarios and writes
...
beforeAll(async () => {
  ...
  tmpDir = await createTempDir({
    prefix: 'fetch-browser',
    unsafeCleanup: true,
  });

  const scriptPath = path.join(
    import.meta.dir,
    'fetch-adapter.browser-script.ts',
  );
...
browserScenarios(
  'FetchAdapter in browser (real server)',
  [
    'adapter: getType returns fetch',
    'adapter: GET 200 with body',
    'adapter: browser manual redirect is opaque and has no detectedRedirectURL',
    'adapter: request headers are forwarded',
    'adapter: upload progress fires 0% first then 100% last',
    'adapter: download progress fires 100% on completion',
    'adapter: opaque redirect still emits terminal progress',
  ],
  () => suite,
);

// ---------------------------------------------------------------------------
// HTTPClient + FetchAdapter end-to-end scenarios
// ---------------------------------------------------------------------------

browserScenarios(
  'HTTPClient + FetchAdapter end-to-end (real server)',
  [
    'client: GET /api/users/:id returns user JSON',
    'client: browser redirect_disabled keeps detectedRedirectURL undefined',
    'client: plain text response has isText',
    'client: malformed JSON sets isParseError and preserves text body',
    'client: synthetic fetch progress still fires callbacks',
    'client: adapterType is fetch',
  ],
  () => suite,
);
