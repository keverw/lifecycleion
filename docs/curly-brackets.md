...
Each path segment resolves only an **own enumerable property**. Inherited properties such as the usual `constructor` and `__proto__`, and values added to `Object.prototype`, are treated as missing; an own enumerable property explicitly supplied with one of those names remains available. This keeps template lookup on the same property surface the logger's redaction walk can inspect. Standard `Error` fields (`name`, `message`, `stack`, and `cause`) are the exception, so documented patterns such as `{{error.message}}` continue to work even though JavaScript defines some of those fields as non-enumerable or on `Error.prototype`.
