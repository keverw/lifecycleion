...
- `beforeResolveOrReject?` - `(action: 'resolve' | 'reject', valueOrReason: unknown) => void | Promise<void>` - Optional callback invoked just before the promise is settled. Useful for logging or side effects. Errors in this callback are caught and reported on the global `'error'` event channel rather than propagating.
