...
A `LoggerService` that throws while the request is being logged does not fail the request either: every log line on the request path is guarded and reported through the global error channel, escalation bookkeeping still runs, and the acknowledgement still resolves. `void triggerShutdown()` is therefore safe when you do not need the acknowledgement.
