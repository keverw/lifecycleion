...
Rendering and redaction failures use that same channel when you have not set a handler, so a `logger.registerReportErrorListener()` records them like any other reported failure. The `Logger` and its sinks never take that route for their own work — they always write to `console.error` instead — because reporting from inside a log call would be logged, and logging renders. **If your own sink, formatter or transformer calls `stringifyValue()` or `errorToString()` directly, pass it an `onFormatError`**: it runs inside a log call while looking like a standalone one, so left to the default it can cycle. See [Where Failures Go](./docs/logger.md#where-failures-go).
